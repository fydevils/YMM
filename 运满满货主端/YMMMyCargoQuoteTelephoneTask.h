//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "YMMPublishCommonNetworkTask.h"

@class NSNumber, NSString;

@interface YMMMyCargoQuoteTelephoneTask : YMMPublishCommonNetworkTask
{
    NSNumber *_bargainId;
    NSString *_callerRealTel;
}

@property(copy, nonatomic) NSString *callerRealTel; // @synthesize callerRealTel=_callerRealTel;
@property(retain, nonatomic) NSNumber *bargainId; // @synthesize bargainId=_bargainId;
- (void).cxx_destruct;
- (Class)ymm_responseModelClass;
- (id)ymm_networkAPIPath;

@end

