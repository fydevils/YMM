//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class EaseMessageModel, NSDictionary, NSString, YFConversation;

@protocol IConversationModel <NSObject>
@property(nonatomic) long long conversationType;
@property(readonly, nonatomic) EaseMessageModel *latestMessage;
@property(retain, nonatomic) NSDictionary *ext;
@property(readonly, nonatomic) YFConversation *yconversation;
@property(retain, nonatomic) NSString *conversationId;
- (id)initWithYFConversation:(YFConversation *)arg1;
@end

